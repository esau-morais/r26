...
const SERVER = process.env.R26_SERVER ?? "r26.fly.dev";
const isDev = SERVER.startsWith("localhost") || SERVER.startsWith("127.0.0.1");
const WS_URL = isDev ? `ws://${SERVER}` : `wss://${SERVER}`;
const API_URL = isDev ? `http://${SERVER}` : `https://${SERVER}`;
